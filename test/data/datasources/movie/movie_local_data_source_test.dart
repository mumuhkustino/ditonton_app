...
void main() {
  ...
  group('remove watchlist movies', () {
    test('should return success message when remove from database is success',
        () async {
      // arrange
      when(mockDatabaseHelper.removeWatchlistMovies(testMovieTable.id))
          .thenAnswer((_) async => 1);
      // act
      final result = await dataSource.removeWatchlistMovies(testMovieTable.id);
      // assert
    ...
    test('should throw DatabaseException when remove from database is failed',
      ...
      when(mockDatabaseHelper.removeWatchlistMovies(testMovieTable.id))
          .thenThrow(Exception());
      // act
      final call = dataSource.removeWatchlistMovies(testMovieTable.id);
      // assert
      expect(() => call, throwsA(isA<DatabaseException>()));
    });
  });
